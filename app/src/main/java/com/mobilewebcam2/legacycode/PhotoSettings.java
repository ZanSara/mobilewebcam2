...
public class PhotoSettings implements SharedPreferences.OnSharedPreferenceChangeListener
{
	...
	public static int getEditInt(Context c, SharedPreferences prefs, String name, int d) throws NumberFormatException
	{
    	...
    	catch(NumberFormatException e)
    	{
    		...
    		if(e.getMessage() != null)
    			msg = e.getMessage();
    		if(com.mobilewebcam2.legacycode.MobileWebCam2.gIsRunning)
    		{
    			try
    			{
    			...
    			com.mobilewebcam2.legacycode.MobileWebCam2.LogE(msg);
    	}
    	return i;
	}
	...
	{
    	mContext = c;
        mPrefs = c.getSharedPreferences(com.mobilewebcam2.legacycode.MobileWebCam2.SHARED_PREFS_NAME, 0);
...
/*		mImprintText = mPrefs.getString("imprint_text", "mobilewebcam " + android.os.Build.MODEL);
    ...
    private static int GetPrefColor(SharedPreferences prefs, String name, String def, int c)
    {
	    ...
	    if(v.length() == 9)
		{
			...
			{
				com.mobilewebcam2.legacycode.MobileWebCam2.LogE("Wrong color string: '" + v + "'");
				e.printStackTrace();
			}
		}
    ...
    public void onSharedPreferenceChanged(SharedPreferences prefs, String key)
	{
    	if(key == "cam_refresh")
    	{
    		...
    		{
    			com.mobilewebcam2.legacycode.MobileWebCam2.LogI(msg);
    		}
    	}
	    		...
	    		try
	    		{
					new AsyncTask<String, Void, Bitmap>()
					{
						...
						protected Bitmap doInBackground(String... params)
						{
							...
							{
								if(e.getMessage() != null)
									com.mobilewebcam2.legacycode.MobileWebCam2.LogE("Imprint picture URL error: " + e.getMessage());
								else
									com.mobilewebcam2.legacycode.MobileWebCam2.LogE("Imprint picture URL invalid!");
								e.printStackTrace();
							}
							catch (IOException e)
							{
								if(e.getMessage() != null)
									com.mobilewebcam2.legacycode.MobileWebCam2.LogE(e.getMessage());
								e.printStackTrace();
							}
	...
	public static void GETSettings(final Context context)
	{
		...
		final SharedPreferences prefs = context.getSharedPreferences(com.mobilewebcam2.legacycode.MobileWebCam2.SHARED_PREFS_NAME, 0);
		...
		if(settingsurl.length() > 0 && gLastGETSettingsPictureCnt < com.mobilewebcam2.legacycode.MobileWebCam2.gPictureCounter && (com.mobilewebcam2.legacycode.MobileWebCam2.gPictureCounter % settingsfreq) == 0)
		{
			gLastGETSettingsPictureCnt = com.mobilewebcam2.legacycode.MobileWebCam2.gPictureCounter;
							...
							try
							{
								...
								if(login.length() > 0)
								{
									...
									catch(Exception e)
									{
										e.printStackTrace();
										if(e.getMessage() != null)
											com.mobilewebcam2.legacycode.MobileWebCam2.LogE("http login " + e.getMessage());
										else
											com.mobilewebcam2.legacycode.MobileWebCam2.LogE("http: unable to log in");
		...
		for(String s : settings)
		{
			try
			{
				...
				if(all.containsKey(param))
				{
					...
					{
						com.mobilewebcam2.legacycode.MobileWebCam2.LogE("Warning: config.txt entry '" + param + "' value is empty!");
					}
				}
			}
			catch(Exception e)
